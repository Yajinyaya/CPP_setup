...
double getExpectation(int k, double a, double b) {
 ...
 for (int i = 0; i < m; i++) {
  ...
  if (i < 2*k) {
   ...
   int howManyOne = i - 2*k;

   for (int j = 0; j < howManyOne; j++) {
    q[j] = 0.0;
   }

   for (int l = 0; l < k; l++) { // for i 
    double coefficient = 0.0;

    for (int o = 0; o < k; o++) { // for j
     if (o < l) {
      coefficient += (2*a + b*q[o]);
     } else if (o > l) {
      coefficient += (2*a - (2*a + b)*q[o]);
     }
    }

    coefficient = coefficient / (k*(k-1)) * (-1);
 ...
 if (lp.lp_type == lp.UNBOUNDED) {
    ...
    //     for (int z = 0; z<n; z++) {
    //         std::cout << "x" << z << " = " << lp.soln[z] << std::endl;
    //     }
    //  } else {
